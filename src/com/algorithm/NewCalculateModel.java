...
public class NewCalculateModel {
	/**   
	 * 
	 * 
	 * 
	...
	public void CalModel(int width,int height, Puma puma_1, Hare hare_1,Map mapInfo) throws IOException{
		...
		for(int i=0; i< height+2; i++){
			for(int j=0; j< width+2; j++){
				puma_halo.getLocation()[i][j]=0;
				hare_halo.getLocation()[i][j]=0;
			}
		}
		
		for(int i=0; i< height; i++){
			for(int j=0; j< width; j++){
				puma_halo.getLocation()[i+1][j+1]=puma_1.getLocation()[i][j];
				hare_halo.getLocation()[i+1][j+1]=hare_1.getLocation()[i][j];
		...
		for(double t = 0; t < 500; ){
			for(int i = 0; i < height; i++){
				...
				for(int j = 0; j < width; j++){
					...
					int numOfNeighbor = 0;  //number of neighbour of land but not water
					int num_water = 0;
					if(puma_halo.getLocation()[i+1][j+1] == 0 || hare_halo.getLocation()[i+1][j+1] == 0)  //if water area then continue
						continue;
					if(height==1 & width==1){
					...
					numOfNeighbor = neigh[i][j];
					tempPuma = puma_halo.getLocation()[i+1][j]+puma_halo.getLocation()[i][j+1]+puma_halo.getLocation()[i+2][j+1]+puma_halo.getLocation()[i+1][j+2];
					tempHare = hare_halo.getLocation()[i+1][j]+hare_halo.getLocation()[i][j+1]+hare_halo.getLocation()[i+2][j+1]+hare_halo.getLocation()[i+1][j+2];
					
					tempPuma = tempPuma - numOfNeighbor*puma_halo.getLocation()[i+1][j+1]; //algorithm calculate for puma
					tempHare = tempHare - numOfNeighbor*hare_halo.getLocation()[i+1][j+1]; //algorithm calculate for hare
					
					puma_2.getLocation()[i][j]= puma_halo.getLocation()[i+1][j+1]+timeStep*(puma_halo.getBirthRate()*hare_halo.getLocation()[i+1][j+1]*puma_halo.getLocation()[i+1][j+1]-puma_halo.getMortalityRate()*puma_halo.getLocation()[i+1][j+1]+puma_halo.getDiffusionRate()*tempPuma);
					hare_2.getLocation()[i][j]= hare_halo.getLocation()[i+1][j+1]+timeStep*(hare_halo.getBirthRate()*hare_halo.getLocation()[i+1][j+1]-hare_halo.getPredationRate()*hare_halo.getLocation()[i+1][j+1]*puma_halo.getLocation()[i+1][j+1]+hare_halo.getDiffusionRate()*tempHare);
		...
		//print part
			...
			for(int i=0;i<height; i++){
				for(int j=0; j< width; j++){
					tmp_puma.getLocation()[i][j]=puma_halo.getLocation()[i+1][j+1];
					tmp_hare.getLocation()[i][j]=hare_halo.getLocation()[i+1][j+1];
					puma_halo.getLocation()[i+1][j+1] = puma_2.getLocation()[i][j];
					hare_halo.getLocation()[i+1][j+1] = hare_2.getLocation()[i][j];
					puma_2.getLocation()[i][j]=tmp_puma.getLocation()[i][j];
					hare_2.getLocation()[i][j]=tmp_hare.getLocation()[i][j];
				}
			}
					//puma_1 = puma_2;
					//puma_2 = tmp_puma;
